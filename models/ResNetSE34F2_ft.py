...
class ResNetSE(nn.Module):
    ...
    def forward(self, x):
        ...
        t = x.size()[-1]
        #print("T", t, x.shape,  torch.mean(x, dim=2, keepdim=True).repeat(1, 1, t), torch.sqrt(torch.var(x, dim=2, keepdim=True).clamp(min=1e-4, max=1e4).repeat(1, 1, t)).shape)
        #400, 128, 51 / 400, 128, / 400, 128 => T, 51,  B X 64 X 51, B X 64 X 51, B X 64, 51
        if self.context:
            global_x = torch.cat(
                (
                    x,
        ...
        # => B X 64 X 51  B X 64, B X 64, B X 128, B X 192 X 51
        
        #print("4", x.shape) #B,256 [400Frame: B X 3072] => B X 128
        x = self.bn5(x)
        #print("5", x.shape) #B X 256 [400Frame: B X 3072] => B X 128
        x = self.fc6(x)
        #print("6", x.shape) #B X 512 [400Frame: B X 256] => B X nOut (256으로 수정?)
        if self.out_bn:
            x = self.bn6(x)
        return x
